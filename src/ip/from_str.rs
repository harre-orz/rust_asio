...
use super::{IpAddr, IpAddrV4, IpAddrV6, LlAddr};
use error::ADDRESS_FAMILY_NOT_SUPPORTED;
use std::ffi::CStr;
use std::io;
    ...
    fn parse<'a>(&self, it: Chars<'a>) -> Result<(Self::Output, Chars<'a>)> {
        if let Ok((_, it)) = Lit('%').parse(it.clone()) {
            if let Ok((ch, mut it)) = Char("a-zA-Z").parse(it.clone()) {
                ...
                vec.push(0);
                let id = unsafe {
                    let name = CStr::from_bytes_with_nul_unchecked(&vec);
...
impl FromStr for IpAddrV4 {
    ...
    fn from_str(s: &str) -> io::Result<IpAddrV4> {
        if let Ok((bytes, _)) = Eos(Sep4By(Dec8, Lit('.'))).parse(s.chars()) {
            Ok(IpAddrV4 { bytes: bytes })
        } else {
            Err(ADDRESS_FAMILY_NOT_SUPPORTED.into())
        }
    }
...
impl FromStr for IpAddrV6 {
    ...
    fn from_str(s: &str) -> io::Result<IpAddrV6> {
        if let Ok(((bytes, scope_id), _)) = Eos(Cat(IpV6, ScopeId)).parse(s.chars()) {
            Ok(IpAddrV6::with_scope_id(
                bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6], bytes[7], scope_id,
            ))
        } else {
            Err(ADDRESS_FAMILY_NOT_SUPPORTED.into())
        }
...
fn test_ipaddr_v6() {
    ...
    assert_eq!(IpAddrV6::from_str("::").unwrap(), IpAddrV6::default());
    assert_eq!(
    ...
    assert_eq!(
        ...
        IpAddrV6::v4_mapped(IpAddrV4::default())
    );
    ...
    if cfg!(target_os = "linux") {
        ...
        assert!(IpAddrV6::from_str("1:2:3:4:5:6:7:8%lo0").unwrap().scope_id() != 0);
    }
}
...
fn test_ipaddr() {
    ...
    assert_eq!(IpAddr::from_str("::").unwrap(), IpAddr::V6(IpAddrV6::default()));
    assert_eq!(
