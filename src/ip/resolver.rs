...
mod ffi {
    ...
    use crate::error::ResolverError;
    use crate::IoContext;
    use crate::ip::{IpEndpoint, IpProtocol, ResolverQuery};
    use crate::socket_base::{Endpoint, Protocol};
        ...
        P: Protocol,
    {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            if self.ai.is_null() {
                ...
                let mut sa = MaybeUninit::<crate::socket_base::SockAddr>::uninit();
                let dst = sa.as_mut_ptr() as *mut u8;
                unsafe { src.copy_to(dst, ai.ai_addrlen as usize) };
                //let sa = unsafe { sa.assume_init() };
                let sa = unsafe { crate::socket_base::SockAddr::init(sa, ai.ai_addrlen) };
                Some(crate::socket_base::new(sa))
            }
        }
    }
...
mod ffi {
    ...
    pub struct ResolverIter<P> {
        base: NonNull<WinSock::ADDRINFOW>,
        ai: *mut WinSock::ADDRINFOW,
        _ctx: IoContext,
        _marker: PhantomData<P>,
    }
    ...
    impl<P> Resolver<P> {
        pub fn new_priv(ctx: &IoContext, pro: P) -> Self {
        todo!()
        }
...
pub use self::ffi::{ResolverIter, Resolver};